...
from PyQt5.QtCore import QThread, pyqtSignal, QEventLoop, QTimer, QCoreApplication
...
from views.dialogs import FolderExistsDialog
...
class FolderScanner(QThread):
    ...
    # Nouveau signal pour demander le choix utilisateur lors d'un conflit de dossier
    request_folder_conflict_decision = pyqtSignal(str, str, object)
    ...
    def _get_user_folder_conflict_decision(self, folder_name: str, parent_path: str) -> str:
        """
        Demande au thread principal le choix utilisateur pour un conflit de dossier.
        Retourne 'use_existing', 'create_new' ou 'cancel'.

        Note: Cette méthode utilise maintenant une approche directe sans dialogue modal
        pour éviter les problèmes de threading, et vérifie la configuration utilisateur.
        """
        print(f"[FolderScanner] Conflit de dossier détecté pour '{folder_name}' dans '{parent_path}'")
        ...
        if use_existing:
            print(f"[FolderScanner] Configuration: utiliser les dossiers existants - '{folder_name}'")
            return 'use_existing'
        else:
            print(f"[FolderScanner] Configuration: créer de nouveaux dossiers - '{folder_name}'")
            return 'create_new'
    ...
    def _create_drive_structure(self, root_path: str, parent_id: str,
        ...
        retry_count = 3
        try:
            existing_folders = self.drive_client.find_folder_by_name_in_parent(parent_id, main_folder_name)
            use_existing = False
            if existing_folders:
                user_choice = self._get_user_folder_conflict_decision(main_folder_name, root_path)
                if user_choice == 'use_existing':
                    main_folder_id = existing_folders[0]['id']
                    use_existing = True
                elif user_choice == 'create_new':
                    use_existing = False
                else:
                    raise Exception("Création de dossier annulée par l'utilisateur.")
            if not use_existing:
        ...
        for subfolder_rel_path in all_folders_to_create:
            ...
            subfolder_id = None
            try:
                existing_folders = self.drive_client.find_folder_by_name_in_parent(parent_drive_id, subfolder_name)
                use_existing = False
                if existing_folders:
                    user_choice = self._get_user_folder_conflict_decision(subfolder_name, os.path.join(root_path, parent_rel_path))
                    if user_choice == 'use_existing':
                        ...
                        # User cancelled, use parent folder as fallback
                ...
                if not use_existing:
                    ...
                    for attempt in range(retry_count):
                        try:
                            ...
                            print(f"⚠️ Retry {attempt+1}/{retry_count} - Failed to create folder '{subfolder_name}': {e}")
                            if attempt == retry_count - 1:
                                # All retries failed
                                raise Exception(f"Failed to create folder '{subfolder_name}' after {retry_count} attempts: {e}")
                            time.sleep(1)

                # Only proceed if we have a valid folder ID
                if subfolder_id:
                    ...
                    # Fallback to parent folder if something went wrong
                    folder_mapping[subfolder_rel_path] = parent_drive_id
                    print(f"⚠️ Using parent folder as fallback for '{subfolder_name}'")
            ...
            # Small delay between folder creations to avoid rate limiting
            time.sleep(0.05)
